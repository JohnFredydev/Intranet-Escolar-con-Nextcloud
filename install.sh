...
show_summary() {
  ...
  echo ""
  echo "  ${ICON_BULLET} Nextcloud:    ${CYAN}${BOLD}http://localhost:8080${NC}"
  echo "  ${ICON_BULLET} Uptime Kuma:  ${CYAN}${BOLD}http://localhost:3001${NC}"
  echo ""
  ...
  echo ""
  echo "  ${DIM}#${NC} Ver estado de contenedores"
  echo "  ${CYAN}docker compose ps${NC}"
  echo ""
  echo "  ${DIM}#${NC} Ver logs en tiempo real"
  echo "  ${CYAN}docker compose logs -f app${NC}"
  echo ""
  echo "  ${DIM}#${NC} Ejecutar comandos OCC"
  echo "  ${CYAN}docker compose exec -u www-data app php occ status${NC}"
  echo ""
  echo "  ${DIM}#${NC} Hacer backup"
  echo "  ${CYAN}bash scripts/backup.sh${NC}"
  echo ""
